...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'biller_type') THEN
    CREATE TYPE teka.biller_type AS ENUM (
...
ALTER TABLE teka.biller_profiles
  ADD COLUMN IF NOT EXISTS biller_type teka.biller_type NOT NULL DEFAULT 'Others',
...
CREATE TABLE teka.bill_payment_details (
    transaction_id BIGINT PRIMARY KEY REFERENCES teka.transactions(transaction_id) ON DELETE CASCADE,
