        ...
        {data.totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50/80 px-5 py-3">
            <p className="text-xs font-medium text-gray-500">
              Page <span className="font-semibold text-gray-800">{data.page}</span> of {data.totalPages}
            </p>
            <div className="flex gap-2">
              <button
                disabled={data.page <= 1}
                onClick={() => updateParams("page", String(data.page - 1))}
                className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-600 shadow-sm hover:bg-gray-50 disabled:opacity-40"
              >
                Previous
              </button>
              <button
