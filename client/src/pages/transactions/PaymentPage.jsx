...
import { CheckIcon, ShoppingBagIcon, ArrowLeftIcon, InformationCircleIcon } from '@heroicons/react/24/outline';
          ...
          <div className="lg:col-span-5 space-y-8 animate-in slide-in-from-left-4 duration-700">
            <div>
              <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary-50 text-primary-600 shadow-sm">
                <ShoppingBagIcon className="h-8 w-8" strokeWidth={2} />
              </div>
              <h1 className="text-3xl font-bold tracking-tight text-gray-900 md:text-4xl">Merchant Payment</h1>
              <p className="mt-4 text-lg font-medium text-gray-500 leading-relaxed">
                Make quick and secure payments at any authorized TekaPakhi merchant outlet. Enter the Merchant's phone number to pay.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4 rounded-2xl border border-primary-50 bg-primary-50/30 p-5">
                ...
                <div>
                  <p className="text-sm font-semibold text-primary-900 uppercase tracking-wider mb-1">Zero Fee for You</p>
                  <p className="text-[15px] font-medium text-primary-800 opacity-80">Merchant payments are completely free for customers. The merchant pays the transaction fee.</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {step === 'form' && (
                <div className="space-y-8">
                  ...
                  <div className="space-y-3">
                    <label className="block text-[11px] font-bold uppercase tracking-[0.15em] text-gray-400 px-1">Merchant Phone Number</label>
                    ...
                    {recipient && (
                      <div className="flex items-center gap-3 rounded-2xl bg-emerald-50 px-4 py-3 border border-emerald-100 animate-in zoom-in-95 duration-300">
                        ...
                        <span className="text-[15px] font-bold text-emerald-700">{recipient.fullName} (Merchant)</span>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Amount Input */}
                  <div className="space-y-3">
                    <label className="block text-[11px] font-bold uppercase tracking-[0.15em] text-gray-400 px-1">Payment Amount</label>
                  ...
                  {/* Note Input */}
                  <div className="space-y-3">
                    <label className="block text-[11px] font-bold uppercase tracking-[0.15em] text-gray-400 px-1">Payment Reference (Optional)</label>
                    <input
                      type="text"
                      value={form.note}
                      onChange={(e) => setForm(p => ({ ...p, note: e.target.value }))}
                      placeholder="e.g., Invoice #123, Order ID"
                  ...
                  >
                    {loading ? <LoadingSpinner size="sm" /> : 'Review Payment'}
                  </button>
                </div>
              )}
              ...
              {step === 'review' && preview && (
                <div className="space-y-10 animate-in zoom-in-95 duration-300">
                  <div className="text-center">
                    <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-2">Total Amount to Pay</p>
                  ...
                  <div className="space-y-4 rounded-3xl bg-gray-50/50 p-8">
                    ...
                    <div className="flex justify-between items-center text-[15px]">
                      ...
                      <span className="font-bold text-gray-400 italic">Free for you</span>
                    </div>
