...
const flowSteps = [
  { key: "recipient", label: "Merchant", hint: "Verify merchant" },
...
export default function PaymentPage() {
  ...
  const fetchRecipients = async () => {
    ...
    try {
      ...
      const merchants = data.data.filter(
        (r) => r.target_type?.toLowerCase() === "merchant",
      );
      setSavedRecipients(merchants);
    } catch (e) {
      console.error("Failed to load saved merchants", e);
    } finally {
  ...
  const fetchRecentRecipients = async () => {
    ...
    try {
      ...
      console.error("Failed to load recent payment contacts", e);
    } finally {
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      setLookupError(
        error.response?.data?.message || "No merchant found with this phone number.",
      );
      setRecipient(null);
    } finally {
      setLoading(false);
    ...
    >
      <div className="mb-8 flex items-center gap-4">
        ...
        <div className="ml-auto flex h-8 w-8 items-center justify-center rounded-full bg-primary-50 text-primary-600">
          <ShoppingBagIcon className="h-4 w-4" strokeWidth={2} />
        </div>
      </div>
        ...
        <div className="flex flex-1 flex-col animate-in slide-in-from-right-4 duration-300">
          ...
          <div className="relative mb-8 mt-8">
            <div className="mb-1 flex items-end justify-between">
              <label className="px-1 text-[12px] font-bold text-gray-500">Payment reference</label>
          ...
          >
            {loading ? <LoadingSpinner size="sm" /> : "Confirm Payment"}
          </button>
        </div>
      )}
    </TransactionFlowLayout>
